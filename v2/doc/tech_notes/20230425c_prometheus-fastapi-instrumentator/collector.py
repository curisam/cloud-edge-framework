...
class RandomNumberCollector(object):
    ...
    def collect(self):
        ...
        gauge = GaugeMetricFamily("random_number_collector_1a", "A random number generator, I have no better idea", labels=["randomNum"])
        gauge.add_metric(['random_num'], random.randint(1, 20))
        yield gauge
        
        count = CounterMetricFamily("random_number_collector_1b", "A random number 2.0", labels=['randomNum'])
